...
- **店舗マスタの再配布禁止**：使用条件で「複製、配布、再公開」が禁止されているため、取得した店舗データを他者に配布しない。なお、ソーシャル機能が送信するのは店舗 ID、店舗名、都道府県、訪問日のみで、店舗マスタ本体（座標、営業時間等）は送信しない（整理は `docs/superpowers/specs/2026-07-07-social-feature-design.md` §4.6 を参照）。
