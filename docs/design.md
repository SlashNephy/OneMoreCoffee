...
開発者本人と、限定配布を受けた少数の友人。公開配布は想定しない（規約上の制約のため、後述）。

配布は Firebase App Distribution 等の招待制の限定配布に限る（公開配布は行わない）。
...
なお、フレンド間のソーシャル共有（リーグ）は 2026-07 に追加された。詳細は `docs/superpowers/specs/2026-07-07-social-feature-design.md` を参照。
...
- **店舗マスタの再配布禁止**：使用条件で「複製、配布、再公開」が禁止されているため、取得した店舗データを他者に配布しない。なお、ソーシャル機能が送信するのは店舗 ID、店舗名、都道府県、訪問日時のみで、店舗マスタ本体（座標、営業時間等）は送信しない（整理は `docs/superpowers/specs/2026-07-07-social-feature-design.md` §4.6 を参照）。
...
- 訪問履歴・店舗データはすべて端末ローカルに保持（ソーシャル機能で送信する項目を除く）
- 外部送信は行わない（クラッシュレポートと、ユーザーが明示的に有効化したソーシャル機能を除く）。ソーシャル機能で送信されるのは店舗 ID、店舗名、都道府県、訪問日、集計統計のみ
...
| フレンド | リーグ情報カード（招待コード）、総制覇数ランキング、アクティビティフィード、退出ボタン |
...
| 訪問済（通常店舗） | 緑塗りつぶし円 |
| 未訪問（通常店舗） | 緑枠のみ円 |
| Reserve店舗 | 緑塗り＋オレンジ枠（訪問状態は塗り/枠で表現） |
| 現在地 | 青塗りつぶし円 |
| クラスタ | 緑円に件数バッジ |
...
4. **タブ構成**：解決済み：フレンドタブとして追加（2026-07）
